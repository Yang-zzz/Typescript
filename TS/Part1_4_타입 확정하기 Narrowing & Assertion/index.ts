...
// 또는 assertion 문법 (타입 덮어쓰기)
// 1. narrowing 할 때 사용
// 2. 유니온타입을 확정을 짖는 문법이지, 타입이 전혀다른 변수에 사용하는 것은 x
// 3. 무슨 타입이 들어올지 확실할 때 사용.
// 결론: 무슨 타입이 들어올지 모를 경우 사용할 수 있음. (ex. 유니온타입) 하지만 확실하게 타입을 알고 있을 때 사용하는게 좋음.

// 숙제1
function test이름1(x?: string) {
  if (x) {
    console.log("안녕하세요" + x);
  } else {
  }
